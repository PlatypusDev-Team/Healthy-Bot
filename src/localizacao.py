...
def processar_e_responder_postos(bot, chat_id, lat, lon, nome_local):
    # Chama a Célula 2
...
def iniciar_localizacao(bot, chat_id):
    usuarios_localizacao.add(chat_id)

    bot.send_message(
        chat_id,
        "📍 Digite seu endereço ou CEP para localizar UBSs próximas."
    )

def processar_localizacao(bot, msg):
    chat_id = msg.chat.id

    lat, lon, nome_local = buscar_coordenadas_por_texto(msg.text)
    ...
    usuarios_localizacao.discard(chat_id)
